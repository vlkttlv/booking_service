...
async def add_room(room_data: SRoomAdd, user: Users = Depends(get_current_user)):
    ...
    new_room_id = await RoomDAO.add(hotel_id=room_data.hotel_id, name=room_data.name, description=room_data.description,
                                    services=room_data.services, quantity=room_data.quantity, price=room_data.price)
    for i in range(1, 4):
